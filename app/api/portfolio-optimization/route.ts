...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔍 Running raw SQL query for user:', userId)
    
    const { data: holdings, error } = await supabase
      .rpc('get_user_portfolios', { user_id_param: userId })

    if (error) {
      console.error('❌ RPC error:', error)
      
      // 🔥 FALLBACK: Try direct query with schema
      const { data: fallbackData, error: fallbackError } = await supabase
        .from('portfolios')
        .select('*')
        .eq('user_id', userId)
      
      console.log('🔍 Fallback holdings found:', fallbackData?.length || 0)
      ...
      if (fallbackData && fallbackData.length > 0) {
        ...
        return processHoldings(fallbackData, userId)
      }
      
      return NextResponse.json({
        message: `No holdings found (RPC error: ${error.message})`,
        recommendations: [],
        pairTrades: [],
    ...
    console.log('✅ Holdings from RPC:', holdings?.length || 0)
    ...
    if (!holdings || holdings.length === 0) {
      ...
      console.log('🔍 Direct holdings found:', directData?.length || 0)
      ...
      if (directData && directData.length > 0) {
        return processHoldings(directData, userId)
      }
      
      return NextResponse.json({
        message: `No holdings found. Please add at least 2 holdings.`,
        recommendations: [],
        pairTrades: [],
        diversificationScore: 0,
        holdingsCount: 0,
    ...
    return processHoldings(holdings, userId)
...
// Helper function to process holdings
async function processHoldings(holdings: any[], userId: string) {
  console.log('🔄 Processing holdings:', holdings.length)
  
  if (holdings.length < 2) {
    return NextResponse.json({
      message: `Add at least 2 holdings to get portfolio optimization advice (you have ${holdings.length})`,
      recommendations: [],
      pairTrades: [],
      diversificationScore: 0,
  ...
  // Fetch current prices and analysis for each holding
  const stockData: any[] = []
